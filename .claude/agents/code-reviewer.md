...
- Does the implementation match what was agreed in the pre-analyze checklist?
...
- Do Zustand stores follow existing patterns in `src/stores/`?
...
- **Critical** — must fix before merge: broken type safety, missing DHF update for a regulated change, untested error paths, architectural constraint violated
- **Important** — should fix: naming inconsistency, missing test coverage, undocumented deviation from plan
