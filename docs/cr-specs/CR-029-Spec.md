...
The CR-driven automation pipeline in WebTPS-DHF (cr-analyze.yml, cr-develop.yml, cr-spec-iterate.yml) was recently enhanced with path fixes and secret configuration updates, but has not been validated in a complete end-to-end scenario. The workflow must prove that:
...
3. Clones product repo (itercharles/WebTPS)
...
### File: `stub-CR-029.md` (Create in WebTPS repo)

**Location:** Root of WebTPS repository (or `/docs/automation-test/`)
...
See specification: [CR-029-Spec.md](../../WebTPS-DHF/docs/cr-specs/CR-029-Spec.md)
...
python -m compliantflow --dhf DHF dhf item get CR-029 | grep status
...
python -m compliantflow --dhf DHF dhf item get CR-029 | grep status
gh pr list --head develop/CR-029 --repo itercharles/WebTPS-DHF
gh pr list --head feat/CR-029 --repo itercharles/WebTPS
```
...
```bash
python -m compliantflow --dhf DHF dhf item get CR-029 | grep status
python -m compliantflow --dhf DHF dhf item list --type srs | grep CR-029
git log --oneline ../WebTPS | grep CR-029
```
...
- Pass `python -m compliantflow --dhf DHF dhf validate schema`
...
python -m compliantflow --dhf DHF dhf item transition CR-029 completed --by "User"
...
| Product repo PR created | ✓ Verify in itercharles/WebTPS |
| CR-029 state is `completed` | ✓ Verify with `python -m compliantflow --dhf DHF dhf item get CR-029` |
| All GitHub Actions jobs succeeded | ✓ Verify with `gh run list --repo itercharles/WebTPS-DHF` |
...
CR-029-Spec.md merged to main
  ...
  → clone product repo (itercharles/WebTPS)
